...
import { NodemcuGerenciaveisService } from 'src/app/service/gerenciaveis/nodemcu.service';
...
export class GerenciaveisComponent implements OnInit, OnDestroy {
  constructor(
    private nodemcuService: NodemcuGerenciaveisService,
    public dialog: MatDialog,
    private mainService: MainGerenciaveisService,
    private modeloService: ModeloGerenciaveisService
  ) { }
  ...
  ngOnInit(): void {
    this.modeloService.getAll().subscribe(res => {
      ...
      if (this.modeloAtual == undefined) {
        this.modeloService.changeIsCurrent('k600', true)
      }
    })
    window.onload = () => {
      this.init = true;
  ...
  impostoIntervalo() {
    ...
    this.mainService.getGerenciaveisRealizadoByDate().subscribe((res) => {
      res.forEach((item) => {
