...
export class MainAmplificador2Service {
  ...
  put(imposto: number, tcimposto: number, shiftTime: number, op: string) {
    ...
    return this.http.put<any>(
       environment.url + 'main_amplificador2/1',{imposto, tcimposto, shiftTime, op}, {headers});
  }
  ...
  getAllRealizado(): Observable<Realizado[]>{
    return this.http.get<Realizado[]>(environment.url + "realizadoHorariaTablet_amplificador2")
  }
  ...
  postControleRealizado(imposto: number, realizado: number, realizadoHora: number, justificativa: string): Observable<RealizadoGeral>{
    return this.http.post<RealizadoGeral>(environment.url + "realizado_amplificador2", {
      "imposto": imposto,
      "realizado": realizado,
