...
export class RelatorioAmplificador2Service {

  constructor(private http:HttpClient) { }

  getGeralNodemcu(startedDate: string, endDate: string): Observable<GeralNodemcu[]>{
    return this.http.get<GeralNodemcu[]>(environment.url + `geral_amplificador2/nodemcu/filterByDate?startDate=${startedDate}¨&endDate=${endDate}`)
  }

  getGeralMain(startedDate: string, endDate: string): Observable<GeralMain[]>{
    return this.http.get<GeralMain[]>(environment.url + `geral_amplificador2/main/filterByDate?startDate=${startedDate}¨&endDate=${endDate}`)
  }

  getGeralRealizadoHoraria(startedDate: string, endDate: string): Observable<GeralRealizadoHoraria[]>{
    return this.http.get<GeralRealizadoHoraria[]>(environment.url + `geral_amplificador2/realizadoHoraria/filterByDate?startDate=${startedDate}¨&endDate=${endDate}`)
  }

  getGeralRealizadoHorariaTablet(startedDate: string, endDate: string): Observable<GeralRealizadoHorariaTablet[]>{
    return this.http.get<GeralRealizadoHorariaTablet[]>(environment.url + `geral_amplificador2/realizadoHorariaTablet/filterByDate?startDate=${startedDate}&endDate=${endDate}`)
  }

  getGeral(startedDate: string, endDate: string): Observable<Geral[]>{
    return this.http.get<Geral[]>(environment.url + `geral_amplificador2/filterByDate?startDate=${startedDate}&endDate=${endDate}`)
  }
  ...
  getGeralVideo(start: string, end: string): Observable<Video[]>{
    return this.http.get<Video[]>(environment.url + `geral_amplificador2/video/filterByDate?startDate=${start}&endDate=${end}`)
  }

  getGeralReproducoes(): Observable<Reproducao[]>{
    return this.http.get<Reproducao[]>(environment.url + 'video_amplificador2')
  }
