...
export class MainInversor2Service {
  ...
  put(imposto: number, tcimposto: number, shiftTime: number, op: string) {
    ...
    return this.http.put<any>(
       environment.url + 'main_inversor2/1',{imposto, tcimposto, shiftTime, op}, {headers});
  }
  ...
  getAllRealizado(): Observable<Realizado[]>{
    return this.http.get<Realizado[]>(environment.url + "realizadoHorariaTablet_inversor2")
  }
  ...
  postControleRealizado(imposto: number, realizado: number, realizadoHora: number, justificativa: string): Observable<RealizadoGeral>{
    return this.http.post<RealizadoGeral>(environment.url + "realizado_inversor2", {
      "imposto": imposto,
      "realizado": realizado,
