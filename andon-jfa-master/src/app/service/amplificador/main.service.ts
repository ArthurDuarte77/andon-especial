...
export class MainAmplificador1Service {
  ...
  put(imposto: number, tcimposto: number, shiftTime: number, op: string) {
    ...
    return this.http.put<any>(
       environment.url + 'main_amplificador/1',{imposto, tcimposto, shiftTime, op}, {headers});
  }
  ...
  getAllRealizado(): Observable<Realizado[]>{
    return this.http.get<Realizado[]>(environment.url + "realizadoHorariaTablet_amplificador")
  }
  ...
  postControleRealizado(imposto: number, realizado: number, realizadoHora: number, justificativa: string): Observable<RealizadoGeral>{
    return this.http.post<RealizadoGeral>(environment.url + "realizado_amplificador", {
      "imposto": imposto,
      "realizado": realizado,
