...
export class MainControleService {
  ...
  put(imposto: number, tcimposto: number, shiftTime: number, op: string) {
    ...
    return this.http.put<any>(
       environment.url + 'main_controle/1',{imposto, tcimposto, shiftTime, op}, {headers});
  }
  ...
  getAllRealizado(): Observable<Realizado[]>{
    return this.http.get<Realizado[]>(environment.url + "realizadoHorariaTablet_controle")
  }
  ...
  postControleRealizado(imposto: number, realizado: number, realizadoHora: number, justificativa: string): Observable<RealizadoGeral>{
    return this.http.post<RealizadoGeral>(environment.url + "realizado_controle", {
      "imposto": imposto,
      "realizado": realizado,
