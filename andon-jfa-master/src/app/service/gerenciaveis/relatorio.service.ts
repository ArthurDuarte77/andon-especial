...
export class RelatorioGerenciaveisService {

  constructor(private http:HttpClient) { }

  getGeralCicloByDate(startedDate: string, endDate: string, name: string): Observable<Ciclo[]>{
    return this.http.get<Ciclo[]>(environment.url + `geral/ciclo_gerenciaveis/filterByDate/${name}?startDate=${startedDate}&endDate=${endDate}`)
  }

  getGeralCiclo(name: string): Observable<Ciclo[]>{
    return this.http.get<Ciclo[]>(environment.url + `geral/ciclo_gerenciaveis/${name}`)
  }

  getGeralNodemcu(startedDate: string, endDate: string): Observable<GeralNodemcu[]>{
    return this.http.get<GeralNodemcu[]>(environment.url + `geral/nodemcu_gerenciaveis/filterByDate?startDate=${startedDate}¨&endDate=${endDate}`)
  }

  getGeralMain(startedDate: string, endDate: string): Observable<GeralMain[]>{
    return this.http.get<GeralMain[]>(environment.url + `geral/main_gerenciaveis/filterByDate?startDate=${startedDate}¨&endDate=${endDate}`)
  }

  getGeralRealizadoHoraria(startedDate: string, endDate: string): Observable<GeralRealizadoHoraria[]>{
    return this.http.get<GeralRealizadoHoraria[]>(environment.url + `geral/realizadoHoraria_gerenciaveis/filterByDate?startDate=${startedDate}¨&endDate=${endDate}`)
  }

  getGeralRealizadoHorariaTablet(startedDate: string, endDate: string): Observable<GeralRealizadoHorariaTablet[]>{
    return this.http.get<GeralRealizadoHorariaTablet[]>(environment.url + `geral_gerenciaveis/realizadoHorariaTablet_gerenciaveis/filterByDate?startDate=${startedDate}&endDate=${endDate}`)
  }

  getGeral(startedDate: string, endDate: string): Observable<Geral[]>{
    return this.http.get<Geral[]>(environment.url + `geral_gerenciaveis/filterByDate?startDate=${startedDate}&endDate=${endDate}`)
  }

  postGeral(body: ResultadoGeral): Observable<ResultadoGeral>{
    return this.http.post<ResultadoGeral>(environment.url + 'geral_gerenciaveis', body)
  }

  getGeralVideo(start: string, end: string): Observable<Video[]>{
    return this.http.get<Video[]>(environment.url + `geral_gerenciaveis/video/filterByDate?startDate=${start}&endDate=${end}`)
  }
