...
export class OperationService {
  ...
  get(name: string): Observable<Operation> {
    return this.http.get<Operation>(environment.url + "operation_controle/" + name)
  }

  post(body: Nodemcu): Observable<Nodemcu> {
    return this.http.patch<Nodemcu>(environment.url + "nodemcu_controle/" + body.nameId.name, body)

  }

  getByName(name: string) {
    return this.http.get<Nodemcu>(environment.url + "nodemcu_controle/" + name)
  }

  getTCimposto(): Observable<Main[]> {
    return this.http.get<Main[]>(environment.url + "main_controle")
  }

  atualizar(id: number, tempo: boolean) :Observable<any>{
    return this.http.put<any>(environment.url + "contadores_controle/" + id + "/" + tempo, {})
  }

  atualizarState(name: string, state: string) {
    this.http.get(environment.url + "nodemcu_controle/atualizarState/" + name + "/" + state).subscribe();
  }

  getTempo(id: number){
    return this.http.get<any>(environment.url + "contadores_controle/" + id)
  }

  getRealizadoHoraria(name: string): Observable<Realizado> {
    return this.http.get<Realizado>(environment.url + "realizadoHorariaTablet_controle/" + name)
  }

  atualizarOcupado(name: string, ocupado: boolean): Observable<Operation> {
    return this.http.get<Operation>(environment.url + `operation_controle/${name}/${ocupado}`)
  }
  ...
  postQrcode(nome: string, cod: string, aprovado: boolean, op: string, execucao: number) {
    ...
    return this.http.post(`${environment.url}qrcode_controle`, {
      "name": nome,
      "cod": cod,
      "data": dataFormatada,
      "hora": horaFormatada,
  ...
  getFonteAtual(): Promise<Modelo> {
    return new Promise((resolve, reject) => {
      ...
      this.http.get<Modelo[]>(environment.url + "fonte_controle").subscribe(res => {
  ...
  changeIsCurrent(modelo: string, isCurrent: boolean): void {
    this.http.get(environment.url + "fonte_controle/" + modelo + "/" + isCurrent).subscribe()
  }

  changeAnalise(nome: string, analise: boolean){
    this.http.get(environment.url + "operation_controle/analise/" + nome + "/" + analise).subscribe()
  }

  changeTimeExcess(name: string){
    this.http.get(environment.url + "nodemcu_controle/timeExcess/" + name).subscribe()
  }
  changeAjuda(name: string){
    this.http.get(environment.url + "nodemcu_controle/ajuda/" + name).subscribe()
  }
  ...
  postVideo(name_id: NameID){
    return this.http.post(environment.url + "geral_controle/video", {nameId: name_id})
  }
