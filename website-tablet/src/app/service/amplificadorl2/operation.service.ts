...
export class OperationService {
  ...
  get(name: string): Observable<Operation> {
    return this.http.get<Operation>(environment.url + "operation_amplificador2/" + name)
  }

  post(body: Nodemcu): Observable<Nodemcu> {
    return this.http.patch<Nodemcu>(environment.url + "nodemcu_amplificador2/" + body.nameId.name, body)

  }

  getByName(name: string) {
    return this.http.get<Nodemcu>(environment.url + "nodemcu_amplificador2/" + name)
  }

  getTCimposto(): Observable<Main[]> {
    return this.http.get<Main[]>(environment.url + "main_amplificador2")
  }

  atualizar(id: number, tempo: boolean) :Observable<any>{
    return this.http.put<any>(environment.url + "contadores_amplificador2/" + id + "/" + tempo, {})
  }

  atualizarState(name: string, state: string) {
    this.http.get(environment.url + "nodemcu_amplificador2/atualizarState/" + name + "/" + state).subscribe();
  }

  getTempo(id: number){
    return this.http.get<any>(environment.url + "contadores_amplificador2/" + id)
  }

  getRealizadoHoraria(name: string): Observable<Realizado> {
    return this.http.get<Realizado>(environment.url + "realizadoHorariaTablet_amplificador2/" + name)
  }

  atualizarOcupado(name: string, ocupado: boolean): Observable<Operation> {
    return this.http.get<Operation>(environment.url + `operation_amplificador2/${name}/${ocupado}`)
  }
  ...
  postQrcode(nome: string, cod: string, aprovado: boolean, op: string, execucao: number) {
    ...
    return this.http.post(`${environment.url}qrcode_amplificador2`, {
      "name": nome,
      "cod": cod,
      "data": dataFormatada,
      "hora": horaFormatada,
  ...
  getFonteAtual(): Promise<Modelo> {
    return new Promise((resolve, reject) => {
      ...
      this.http.get<Modelo[]>(environment.url + "fonte_amplificador2").subscribe(res => {
  ...
  changeAnalise(nome: string, analise: boolean){
    this.http.get(environment.url + "operation_amplificador2/analise/" + nome + "/" + analise).subscribe()
  }

  changeTimeExcess(name: string){
    this.http.get(environment.url + "nodemcu_amplificador2/timeExcess/" + name).subscribe()
  }
  changeAjuda(name: string){
    this.http.get(environment.url + "nodemcu_amplificador2/ajuda/" + name).subscribe()
  }

  getAll(): Observable<Operation[]>{
    return this.http.get<Operation[]>(environment.url + "operation_amplificador2")
  }

  postVideo(name_id: NameID){
    return this.http.post(environment.url + "geral_amplificador2/video", {nameId: name_id})
  }
